...
def post_create():
    ...
    for i in category_list:
        if request.form['category'] == i.get_name():
            ...
            db.session.add(post)
            db.session.commit()
            return render_template('post/create.html', post=post_schema.dumps(post))
...
def post_create_input(topic):
    if topic == "create":
        ...
        return render_template('post/create_input.html', category_list=result)
...
def select_title():
    ...
    return render_template('post/title.html', title=list(set(result)))
...
def post_delete():
    ...
    return render_template('post/delete_result.html', result=post_schema.dumps(Post.query.all(), many=True))
