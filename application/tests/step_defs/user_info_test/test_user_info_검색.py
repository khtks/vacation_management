...
@pytest.fixture
def user(session):
    user = UserInfo(google_id="khtks@naver.com", en_name="Sam", ko_name="Kong", role=True)
    return user
...
def all_user_info(request_uri_all):
    assert request_uri_all.get_data(as_text=True)


# Scenario 2

@scenario('../../features/user_info/user_info_검색.feature', '관리자가 특정 user info 검색')
def test_specific_user_info_검색():
    pass
...
def role_user(session):
    user = session.query(UserInfo).filter_by(role=True).first()
...
@when(parsers.parse("올바른 {uri}에 요청했을 때"))
def request_uri_specific(client, user_is_in_db, uri):
...
def check_user_info(general_user, request_uri):
    assert user_info_schema.dumps(general_user) == request_uri


# Scenario 4

@scenario('../../features/user_info/user_info_검색.feature', '일반 사용자가 다른 사람의 user info 검색')
def test_no_authority_검색():
    pass


@given("일반 사용자의 계정이어야 하고")
def user_is_general(session):
    user = UserInfo.query.filter(UserInfo.role == False).first()
    assert user
...
def request_no_authorize(client, users_in_db, user_is_general, uri):
    uri = uri + str(users_in_db.get_id())
