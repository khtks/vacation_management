...
def general_user(session):
    try:
        general_user = UserInfo.query.filter_by(admin=False).first()
        assert general_user

    except BaseException:
        general_user = UserInfo(google_id="konghs@naver.com", en_name="Hammington", ko_name="Hee San", admin=False)
...
def admin_user(session):
    try:
        admin_user = UserInfo.query.filter_by(admin=True).first()
        assert admin_user

    except BaseException:
        admin_user = UserInfo(google_id="khtks@naver.com", en_name="Sam", ko_name="Kong", admin=True)
...
@scenario('../../features/user_info/user_info_수정.feature', 'user info 수정')
def test_사용자_정보_수정():
    pass
...
def modify_user_info(client, admin_user, general_user, uri, session):
    ...
    response = client.put(uri, query_string=dict(id = admin_user.id, google_id="google_id", ko_name="ko_name", en_name="en_name", admin=1))

    assert response.status_code == 200
    yield user_info_schema.load(response.json, session=session)
...
@scenario('../../features/user_info/user_info_수정.feature', '자신의 정보 수정')
def test_자신의_정보_수정():
    pass
...
def modify_own_info(client, uri, session, admin_user):
    ...
    response = client.put(uri, query_string=dict(id=admin_user.id, google_id="own_id", ko_name="ko_name", en_name="en_name", admin=0))
...
@scenario('../../features/user_info/user_info_수정.feature', '수정할 user가 관리자인 경우 수정 불가')
def test_관리자_수정_실패():
    pass
...
def another_admin(client, session):
    response = client.post('/user/info/', data=dict(google_id="g_id", en_name="ename", admin=1))

    assert len(UserInfo.query.filter_by(admin=True).all()) >= 2
    return user_info_schema.load(response.json, session=session)
...
def modify_admin_user(client, uri, session, admin_user, another_admin):
    uri = uri + str(admin_user.id)
    response = client.put(uri, query_string=dict(id=another_admin.id, google_id="modify_google", en_name="modify_name", admin=0))
...
@scenario('../../features/user_info/user_info_수정.feature', '일반 사용자가 admin을 수정하려는 경우 실패')
...
def modify_own_admin(client, uri, session, general_user):
    uri = uri + str(general_user.id)
    response = client.put(uri, query_string=dict(id=general_user.id, google_id="modify_google", en_name="modify_name", admin=1))
...
@scenario('../../features/user_info/user_info_수정.feature', 'DB의 조건을 위배해 수정 불가')
def test_조건위배_수정_실패():
    pass
...
def unconstrained_value(client, session, uri, general_user, admin_user):
    ...
    uri = uri + str(general_user.id)
    response = client.put(uri, query_string=dict(id=admin_user.id, google_id="modified_google_id", en_name="modified_en_name", ko_name="test_name", admin=1))
